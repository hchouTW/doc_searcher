...
def run_python(code, *flags):
    ...
    return subprocess.run(
        [sys.executable, *flags, "-c", code], cwd=SRC_DIR.parent / "tests",
        capture_output=True, text=True, env=env,
    )


@pytest.mark.skipif(not SRC_DIR.is_dir(), reason="needs the source checkout (-S hides site-packages)")
...
def test_package_imports_without_repository_root_on_sys_path():
    ...
    result = subprocess.run(
        [sys.executable, "-I", "-c", "import doc_searcher, doc_searcher.cli; print(doc_searcher.__file__)"],
        cwd=SRC_DIR.parent.parent, capture_output=True, text=True,
    )
