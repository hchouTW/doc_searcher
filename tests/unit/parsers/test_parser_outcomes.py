...
@pytest.mark.parametrize("raw, expected", [
    ...
    pytest.param("预算报告".encode("gbk"), "预算报告", marks=pytest.mark.xfail(
        strict=True, reason="Known limitation: BOM-less GBK is often also valid Big5, and Big5 "
...
# ------------------------------------------------------ corrupt / encrypted
@pytest.mark.parametrize("name", ["bad.pdf", "bad.docx", "bad.xlsx", "bad.pptx", "bad.xls", "bad.ppt", "bad.doc"])
@pytest.mark.parametrize("content", [b"", b"\x00\x01garbage not a document" * 40], ids=["zero-bytes", "garbage"])
...
def test_user_password_pdf_is_encrypted(tmp_path):
    import pymupdf
    path = make_pdf(tmp_path / "locked.pdf", "secret budget",
...
def test_owner_password_only_pdf_is_still_readable(tmp_path):
    import pymupdf
    path = make_pdf(tmp_path / "restricted.pdf", "restricted budget",
                    encryption=pymupdf.PDF_ENCRYPT_AES_256, owner_pw="owner",
...
@pytest.mark.parametrize("module, name, build", [
...
# --------------------------------------------------------------- paths
@pytest.mark.parametrize("name", ["報告 最終版 (1).pdf", "Résumé 2026.docx", "予算 表.xlsx", "간단한 발표.pptx", "notes with spaces.txt"])
...
def test_pdf_document_is_closed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pymupdf.Document, "close", lambda self: (closed.append(True), real_close(self)))
    parse_file(str(make_pdf(tmp_path / "a.pdf", "text")))
    parse_file(str(make_pdf(tmp_path / "b.pdf", "x", encryption=pymupdf.PDF_ENCRYPT_AES_256, user_pw="u", owner_pw="o")))
    assert len(closed) >= 2
